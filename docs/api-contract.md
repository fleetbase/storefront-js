...
- ESM externalizes `@fleetbase/sdk` to preserve class identity. The CommonJS build temporarily bundles it because upstream advertises `dist/cjs/fleetbase.js` from a `type: module` package; a clean consumer cannot reliably execute that target. The packed CommonJS fixture prevents removing this compatibility boundary before upstream publishes a genuine `.cjs` entry.
