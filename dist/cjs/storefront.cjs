...
class StoreHour extends StorefrontResource {
    ...
    get is24Hours() {
        ...
        if (start === null || end === null || !dateFns.isValid(start) || !dateFns.isValid(end)) {
            return false;
        }
    ...
    get startDateInstance() {
        ...
        return dateFns.parse(start, format, new Date());
    }
    get endDateInstance() {
        ...
        const format = includesSeconds ? 'HH:mm:ss' : 'HH:mm';
        return dateFns.parse(end, format, new Date());
    }
    get humanReadableHoursRange() {
        ...
        return `${dateFns.format(start, 'p')} - ${dateFns.format(end, 'p')}`;
    }
...
class StoreLocation extends StorefrontResource {
    ...
    get hours() {
        ...
        return new at((hours || []).map((attributes) => new StoreHour(attributes, this.adapter)));
    }
    ...
    get today() {
        const today = dateFns.format(new Date(), 'EEEE');
...
class Store extends StorefrontResource {
    ...
    getLocations(params = {}) {
        return this.adapter.get('locations', { store: this.id, ...params }).then((storeLocations) => {
            return new at(storeLocations.map((attributes) => {
                if (attributes.place) {
    ...
    getPaymentGateways(params = {}) {
        return this.adapter.get('gateways', params).then((gateways) => {
            return new at(gateways.map((attributes) => {
